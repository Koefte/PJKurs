...
//throttle pwm values
const int rcin_cfg_thro_low      = 1010; //used to set rcin_thro_is_low flag when pwm is below. Note: your craft won't arm if this is too low.
...
float subTrim1=0.3;                   //levelout servo1
...
float altitudeBarometer,pressureInitialBaro;
float LoopTimer;
float baro_pressure;
const int NUM_READINGS = 1; // Number of readings to average
float altitudeReadings[NUM_READINGS]; // Array to store altitude readings
int indexBar = 0; // Index to keep track of the current position in the array
...
void altholdSetup(){
  ...
  for(int i=0;i<2000;i++){
    baro.update();
    pressureInitialBaro+=baro.press_pa;
  }
  pressureInitialBaro/=2000;
  F = {1, 0.004,
            0, 1};  
  G = {0.5*0.004*0.004,
            0.004};
...
float getAlt() {
  baro.update();
  float currHeight = 44330 * (1 - pow(baro.press_pa / pressureInitialBaro, 1 / 5.255)) * 100;
  return currHeight;
}
...
void imu_loop() {
  ...
  control_Angle(false); //Stabilize on pitch/roll angle setpoint, stabilize yaw on rate setpoint
...
void control_Mixer() {
  ...
  // mixing Motors
  out_command[MOTOR1] = rcin_thro + transitionFadeValue*(roll_PID + pitch_PID) + (1-transitionFadeValue)*yaw_PID;
  out_command[MOTOR2] = rcin_thro - transitionFadeValue*(roll_PID + pitch_PID) - (1-transitionFadeValue)*yaw_PID;
