...
const int numParts = 4; // Number of parts to smooth out
...
const float maxChangePerCycleYaw = 0.02; // Maximum change allowed per cycle for yaw control
float previousYawOutput = 0.5; // Track the previous output value for yaw control




float smoothedValues[numParts][PPM_CHANNELS] = {0}; // Array to store smoothed values
...
float pidTHROCon(float tHeight, float cHeight) {
    ...
    float newOutput = throhoverpos + error_scaled / 4;
    ...
    // Clamp the output within the boundaries of 0.3 and 0.4
    newOutput = constrain(newOutput, 0.33, 0.38);
